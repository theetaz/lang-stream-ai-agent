...
async def upload_file(
    ...
    uploaded_file = await file_service.upload_file(db, current_user.id, file, session_id)
...
async def get_files(
    ...
    files = await file_service.get_user_files(db, current_user.id, session_id, limit, offset)
    return success_response(
...
async def get_file(
    ...
    return success_response(
        UploadedFileResponse.model_validate(file),
