...
async def _setup_checkpointer_with_autocommit():
    ...
    conn = await AsyncConnection.connect(
        conninfo=settings.psycopg_database_url, autocommit=True
    )
    try:
    ...
    if not _setup_completed:
        ...
        for attempt in range(max_retries):
            try:
                ...
                error_msg = str(e)
                if (
                    "CREATE INDEX CONCURRENTLY" in error_msg
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.info(
                            f"Retrying checkpointer setup (attempt {attempt + 1}/{max_retries})"
                        )
                        continue
                    else:
                        # Last attempt: use autocommit connection
                        logger.info(
                            "Using autocommit connection for checkpointer setup"
                        )
                        try:
                            ...
                            logger.error(
                                f"Checkpointer setup failed even with autocommit: {autocommit_error}"
                            )
                            raise
                elif (
